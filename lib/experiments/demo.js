(function() {
  'use strict';
  var $, $async, $split_tsv, CND, PATH, PSPG, SP, abspath, as_csv, badge, debug, declare, echo, help, info, isa, join_paths, jr, path_1, rpr, select, size_of, to_width, type_of, types, urge, validate, warn, whisper, width_of,
  ...
  $split_tsv = function(...names) {
    ...
    pipeline.push($(function(fields, send) {
      var d, field, i, idx, len, ref;
      d = {};
      for (idx = i = 0, len = fields.length; i < len; idx = ++i) {
        field = fields[idx];
        d[(ref = names[idx]) != null ? ref : `field${idx + 1}`] = field;
      }
      return send(d);
    }));
  ...
  this.demo_many_rows = function() {
    return new Promise((resolve) => {
      var pipeline, source;
      source = SP.new_value_source(this.get_random_words(800));
      pipeline = [];
      pipeline.push(source);
